...
static OpenMenuOS::ButtonReadProvider buttonReadProvider = nullptr;

static int readButtonPin(int pin)
{
  if (buttonReadProvider)
  {
    return buttonReadProvider(pin);
  }

  return digitalRead(pin);
}

static bool usingExternalButtonProvider()
{
  return buttonReadProvider != nullptr;
}
...
#endif
  // Initialize button pins if specified. External providers may use virtual pin IDs.
  if (!usingExternalButtonProvider())
  {
    if (BUTTON_UP_PIN != -1)
    {
...
void Screen::drawSelectionRect()
{
  ...
  {
  case 0:
    if (readButtonPin(BUTTON_SELECT_PIN) == buttonVoltage && config.buttonAnimation)
    {
...
void SettingsScreen::handleInput()
{
  ...
  else
  {
    ...
    if (BUTTON_UP_PIN != -1)
    {
      ...
      int currentUpButtonState = readButtonPin(BUTTON_UP_PIN);
    ...
    {
      // Read current button state
      int currentDownButtonState = readButtonPin(BUTTON_DOWN_PIN);
  ...
  int selectButtonState = readButtonPin(BUTTON_SELECT_PIN);
...
void MenuScreen::handleInput()
{
  ...
  else
  {
    ...
    if (BUTTON_UP_PIN != -1)
    {
      ...
      int upState = readButtonPin(BUTTON_UP_PIN);
    ...
    if (BUTTON_DOWN_PIN != -1)
    {
      ...
      int downState = readButtonPin(BUTTON_DOWN_PIN);
  ...
  int selectState = readButtonPin(BUTTON_SELECT_PIN);
...
void CustomScreen::handleInput()
{
  ...
  if (readButtonPin(BUTTON_SELECT_PIN) == buttonVoltage)
  {
  ...
  else if (readButtonPin(BUTTON_SELECT_PIN) == !buttonVoltage)
  {
    // On button release
    if (selectClicked)
    {
...
void PopupManager::handleInput()
{
  ...
  else
  {
    ...
    int selectButtonState = readButtonPin(BUTTON_SELECT_PIN);
    int upButtonState = readButtonPin(BUTTON_UP_PIN);
...
void OpenMenuOS::setButtonReadProvider(ButtonReadProvider provider)
{
  buttonReadProvider = provider;
}

void OpenMenuOS::clearButtonReadProvider()
{
  buttonReadProvider = nullptr;
}
